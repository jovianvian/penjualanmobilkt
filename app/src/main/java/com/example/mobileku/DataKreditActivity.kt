...
class DataKreditActivity : AppCompatActivity() {
    ...
    private val listKredit = ArrayList<Kredit>()
    private val listKtp = ArrayList<String>()
    private val listKodePaket = ArrayList<String>()
    ...
    private fun loadMasterData() {
        ...
        val reqPaket = JsonArrayRequest(
            "$baseUrl/TampilPaket.php",
            { response ->
                listKodePaket.clear()
                for (i in 0 until response.length()) {
                    val kode = response.getJSONObject(i).optString("kode_paket", "")
                    if (kode.isNotEmpty()) listKodePaket.add(kode)
                }
            },
            {}
        )
        ...
        val request = JsonArrayRequest(
            ...
            { response ->
                ...
                for (i in 0 until response.length()) {
                    ...
                    val item = Kredit(
                        kode = obj.optString("kode_kredit", "-"),
                        ktp = obj.optString("ktp", "-"),
                        kodePaket = obj.optString("kode_paket", "-"),
                        kodeMobil = obj.optString("kode_mobil", "-"),
                        tanggal = obj.optString("tanggal_kredit", "-"),
                        bayar = obj.optString("bayar_kredit", "0"),
                        tenor = obj.optString("tenor", "0"),
                        total = obj.optString("totalcicil", "0")
                    )
                    listKredit.add(item)
                    listData.add(
                        "Kode Kredit: ${item.kode}\nKTP: ${item.ktp}\nKode Paket: ${item.kodePaket}\nKode Mobil: ${item.kodeMobil}\nTanggal: ${item.tanggal}\nBayar: ${item.bayar}\nTenor: ${item.tenor}\nTotal Cicilan: ${item.total}"
                    )
                }
                listView.adapter = ArrayAdapter(this, R.layout.row_data_item, R.id.tvDataItem, listData)
            },
            { Toast.makeText(this, "Gagal ambil data kredit", Toast.LENGTH_SHORT).show() }
        )
        Volley.newRequestQueue(this).add(request)
    }

    private fun showAksiDialog(position: Int) {
        val item = listKredit[position]
        AlertDialog.Builder(this)
            .setTitle("Aksi Data Kredit")
    ...
    private fun showTambahDialog() {
        if (listKtp.isEmpty() || listKodePaket.isEmpty() || listKodeMobil.isEmpty()) {
            Toast.makeText(this, "Data referensi belum siap, coba lagi", Toast.LENGTH_SHORT).show()
            loadMasterData()
            return
        }

        val container = dialogContainer()
        val inputKode = EditText(this).apply { hint = "Kode Kredit" }
        val spinnerKtp = createSpinner(listKtp)
        val spinnerKodePaket = createSpinner(listKodePaket)
        val spinnerKodeMobil = createSpinner(listKodeMobil)
        val inputTanggal = createDateInput("")
        val inputBayar = EditText(this).apply { hint = "Bayar Kredit" }
        val inputTenor = EditText(this).apply { hint = "Tenor" }
        val inputTotal = EditText(this).apply { hint = "Total Cicilan" }

        container.addView(inputKode)
        addLabel(container, "KTP")
        container.addView(spinnerKtp)
        addLabel(container, "Kode Paket")
        container.addView(spinnerKodePaket)
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Simpan") { _, _ ->
                ...
                if (kode.isEmpty() || ktp.isEmpty() || kodePaket.isEmpty() || kodeMobil.isEmpty() || tanggal.isEmpty() || bayar.isEmpty() || tenor.isEmpty() || total.isEmpty()) {
                    Toast.makeText(this, "Semua data wajib diisi", Toast.LENGTH_SHORT).show()
                } else {
                    tambahKredit(kode, ktp, kodePaket, kodeMobil, tanggal, bayar, tenor, total)
                }
            }
    ...
    private fun showEditDialog(item: Kredit) {
        if (listKtp.isEmpty() || listKodePaket.isEmpty() || listKodeMobil.isEmpty()) {
            Toast.makeText(this, "Data referensi belum siap, coba lagi", Toast.LENGTH_SHORT).show()
            loadMasterData()
            return
        ...
        val spinnerKodePaket = createSpinner(listKodePaket, item.kodePaket)
        ...
        val inputTotal = EditText(this).apply {
            hint = "Total Cicilan"
            setText(item.total)
        }
        ...
        AlertDialog.Builder(this)
            .setTitle("Edit Kredit (${item.kode})")
            .setView(container)
            .setPositiveButton("Simpan") { _, _ ->
                val ktp = spinnerKtp.selectedItem?.toString().orEmpty()
                val kodePaket = spinnerKodePaket.selectedItem?.toString().orEmpty()
                ...
                val tenor = inputTenor.text.toString().trim()
                val total = inputTotal.text.toString().trim()
                if (ktp.isEmpty() || kodePaket.isEmpty() || kodeMobil.isEmpty() || tanggal.isEmpty() || bayar.isEmpty() || tenor.isEmpty() || total.isEmpty()) {
                    Toast.makeText(this, "Semua data wajib diisi", Toast.LENGTH_SHORT).show()
                } else {
                    updateKredit(item.kode, ktp, kodePaket, kodeMobil, tanggal, bayar, tenor, total)
                }
            }
            .setNegativeButton("Batal", null)
            .show()
    ...
    private fun konfirmasiHapus(item: Kredit) {
        AlertDialog.Builder(this)
            .setTitle("Hapus Kredit")
            .setMessage("Hapus data kode ${item.kode}?")
            .setPositiveButton("Hapus") { _, _ -> hapusKredit(item.kode) }
            .setNegativeButton("Batal", null)
            .show()
    }

    private fun tambahKredit(
        kode: String,
        ktp: String,
        kodePaket: String,
        kodeMobil: String,
        ...
        val request = object : StringRequest(
            Request.Method.POST,
            "$baseUrl/TambahKreditV2.php",
            { response -> handleMutasiResponse(response, "Data kredit berhasil ditambahkan") },
            ...
            override fun getParams(): MutableMap<String, String> {
                return hashMapOf(
                    "kode_kredit" to kode,
                    "ktp" to ktp,
                    "kode_paket" to kodePaket,
                    "kode_mobil" to kodeMobil,
                    "tanggal_kredit" to tanggal,
                    "bayar_kredit" to bayar,
                    "tenor" to tenor,
                    "totalcicil" to total
                )
            }
    ...
    private fun updateKredit(
        ...
        val request = object : StringRequest(
            Request.Method.POST,
            "$baseUrl/EditKredit.php",
            { response -> handleMutasiResponse(response, "Data kredit berhasil diupdate") },
            { Toast.makeText(this, "Gagal koneksi ke server", Toast.LENGTH_SHORT).show() }
        ) {
            override fun getParams(): MutableMap<String, String> {
                return hashMapOf(
    ...
    private fun hapusKredit(kode: String) {
        val request = object : StringRequest(
            Request.Method.POST,
            "$baseUrl/HapusKredit.php",
            { response -> handleMutasiResponse(response, "Data kredit berhasil dihapus") },
            { Toast.makeText(this, "Gagal koneksi ke server", Toast.LENGTH_SHORT).show() }
        ) {
            override fun getParams(): MutableMap<String, String> {
                return hashMapOf("kode_kredit" to kode)
            }
        }
        Volley.newRequestQueue(this).add(request)
    }
    ...
    private fun createDateInput(initialValue: String): EditText {
        val input = EditText(this).apply {
            hint = "Tanggal Kredit (YYYY-MM-DD)"
            isFocusable = false
            isClickable = true
